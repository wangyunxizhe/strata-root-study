...
/**
 * 该类的主要作用是如果日期不是工作日，则更改日期。
 * <p>
 * 在处理财务中的日期时，通常希望将非工作日（如周末和假日）转换为附近的有效工作日。此类表示这些必要的调整。
 * <p>
 * 此类将{@linkplain BusinessDayConvention business day convention}与
 * {@linkplain HolidayCalendarId holiday calendar}结合在一起。
 * 要调整日期，必须提供{@link ReferenceData}以解析holiday calendar。
 */
@BeanDefinition
public final class BusinessDayAdjustment
    ...
    /**
     * 如果日期不在工作日，则用于调整日期的具体规则。
     * <p>
     * 该规则决定在该日期是假日时是向前还是向后移动。
     */
     ...
     * 使用指定的具体规则和日历获取实例。
     * <p>
     * 调整日期时，将使用指定的日历应用具体规则。如果不是在工作日就调整日期的规则
     *
     * @param convention 如果不是在工作日就调整日期的具体规则
     * @param calendar   定义假日和工作日的日历
     * @return the adjuster
     */
