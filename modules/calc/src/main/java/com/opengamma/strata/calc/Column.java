 ...
 * 在一组计算结果中定义一列。
 * <p>
 * {@link CalculationRunner}提供了为给定的一组目标和列计算结果网格的能力。该类用于定义列。
 * <p>
 * 列是根据一个唯一的名称、要计算的度量和一组控制计算的参数来定义的。
 * 要调用的函数和默认的参数集是在{@link CalculationRules}上定义的。
 */
@BeanDefinition
public final class Column implements ImmutableBean {
     ...
     * 要计算的度量值。
     * <p>
     * 定义了正在执行的计算，比如“PresentValue”或“ParRate”。
     */
    @PropertyDefinition(validate = "notNull")
     ...
     * 报告货币，用于控制货币转换，可选。
     * <p>
     * 这用于指定结果应该报告的货币。如果结果与货币无关，例如“票面利率”，则报告货币将被有效地忽略。
     * <p>
     * 如果为空，则使用来自{@link CalculationRules}的报告货币。
     */
    @PropertyDefinition(get = "optional")
    private final ReportingCurrency reportingCurrency;
    /**
     * 应用于此列的计算参数，用于控制计算的执行方式
     * <p>
     * 如果这里定义了一个参数，
     * 并且在规则中使用相同的{@linkplain CalculationParameter#queryType() query type}定义了一个参数，那么列参数优先。
     * <p>
     * 当构建时，这些将默认为空
     */
    @PropertyDefinition
