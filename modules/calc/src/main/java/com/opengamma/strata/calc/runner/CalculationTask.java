...
import static com.opengamma.strata.collect.Guavate.*;

/**
 * 用于执行计算的单个任务。
 * <p>
 * 这是计算运行程序中的单个执行单元。它由{@link CalculationFunction}和适当的输入组成，
 * 包括一个{@link CalculationTarget}。当调用时，它将为结果网格中的一个或多个列计算结果。
 */
...
public final class CalculationTask implements ImmutableBean {
     ...
     * 获取一个实例，该实例将计算指定的单元格。
     * <p>
     * 所有单元格都必须用于相同的行索引，并且所有列索引都不能重叠。
     *
     * @param target     the target for which the value will be calculated
     * @param function   the function that performs the calculation
     * @param parameters the additional parameters
     * @param cells      这个task要计算的单元格
     * @return the task
     */
    public static CalculationTask of(
            CalculationTarget target,
