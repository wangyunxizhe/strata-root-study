 ...
 * 对一组目标和列执行计算的计算结果。
 * <p>
 * 这定义了一个结果网格，其中网格包含每个目标的一行和每个度量的一列。每个结果可以是单个值或多方案值。
 * 多场景值将实现{@link ScenarioArray}，除非已聚合。
 */
...
public final class Results implements ImmutableBean {
     ...
     * The column headers.
     * <p>
     * 结果中的每一列都由包含名称和度量值的标题定义。此列表的大小定义了解释单元格列表所需的列数。
     */
     ...
     * <p>
     * 此列表包含网格中每个单元格的计算结果。
     * 单元格按目标分组，然后按列分组。因此，给定单元格的索引是{@code (targetRowIndex * columnCount) + columnIndex}。
     * <p>
     * 例如，给定一组具有两个目标（T1和T2）和三列C1、C2和C3的结果，结果将为：
     * <pre>
