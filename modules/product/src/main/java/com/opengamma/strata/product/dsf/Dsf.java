...
/**
 * 可交割swap期货合约。
 * final类，可通过内部类Builder中的构造函数，以bean属性copy的方式，将属性值传递给外部类。
 * 之后再通过Dsf类自身的实例化入口方法build()来获取实例对象。
 * <p>
 * 可交割swap期货是一种金融工具，可以在未来日期实际结算利率掉期。
 * 交付的swap由中央对手方清算。
 * 交割前的最后一个未来价格按基础swap现值报价。
 * 期货产品按天计算保证金。
 * <p>
 * <h4>Price</h4>
 ...
 * <p>
 * Strata在贸易模型、价格和市场数据中对DSF使用十进制价格（decimal prices）。
 * 十进制价格是基于与隐含百分比相等的十进制乘数，因此100.182的市场价格用1.00182表示。
 */
...
public final class Dsf
    ...
    private void validate() {
        ArgChecker.inOrderOrEqual(
                deliveryDate, underlyingSwap.getStartDate().getUnadjusted(),
                "deliveryDate", "underlyingSwap.startDate.unadjusted");
        ArgChecker.isFalse(underlyingSwap.isCrossCurrency(),
                "Underlying swap must not be cross currency");
        for (SwapLeg swapLeg : underlyingSwap.getLegs()) {
            if (swapLeg.getType().equals(SwapLegType.FIXED)) {
                ArgChecker.isTrue(swapLeg.getPayReceive().isReceive(),
                        "Underlying swap must receive the fixed leg");
            }
            if (swapLeg instanceof RateCalculationSwapLeg) {
                RateCalculationSwapLeg leg = (RateCalculationSwapLeg) swapLeg;
                ArgChecker.isTrue(Math.abs(leg.getNotionalSchedule()
                                .getAmount().getInitialValue()) == 1d,
